...
namespace FlexUI
{
	...
	public class SimpleGestures : MonoBehaviour
	{
		...
		public event Action<int, float> onSwipeFromHoriz = delegate { };
		public event Action<int, float> onSwipeFromVert = delegate { };
		...
		public static event Action<int, float> OnSwipeFromHoriz { add { Instance.onSwipeFromHoriz += value; } remove { Instance.onSwipeFromHoriz -= value; } }
		public static event Action<int, float> OnSwipeFromVert { add { Instance.onSwipeFromVert += value; } remove { Instance.onSwipeFromVert -= value; } }
		...
		private Camera cam => Camera == null ? UnityEngine.Camera.main : Camera;
		...
		private static SimpleGestures GetOrCreateInstance()
		{
			if (instance == null)
			{
				Debug.LogWarning($"<color=#ffff00ff>[FlexUI]</color> {nameof(SimpleGestures)} is not found on Scene. It will be created automatically.");
		...
		public static GameObject GetUIObjectUnderPosition(Vector2 pos)
		{
			...
			if (results.Count == 0)
				return null;
			return results.OrderBy(r => -r.depth).First().gameObject;
		}
		...
		public static IEnumerable<RaycastResult> GetUIObjectsUnderPosition(Vector2 pos)
		{
			...
			if (results.Count == 0)
				yield break;
			foreach (var res in results.OrderBy(r => -r.depth))
				yield return res;
		}
		...
		private void ProcessNone1(Vector2 pos)
		{
			...
			if (dPos.sqrMagnitude > SwipeTolerance)
			{
				if (Mathf.Abs(dPos.x) > Mathf.Abs(dPos.y))
				{
					...
					onSwipeFromHoriz((int)Mathf.Sign(dPos.x), startPos[0].x);
					prevPos = startPos[0];
					gesture = Gesture.Pan;
					ProcessPan(pos);
				}
				else
				{
					...
					onSwipeFromHoriz((int)Mathf.Sign(dPos.x), startPos[0].y);
					prevPos = startPos[0];
					gesture = Gesture.Pan;
					ProcessPan(pos);
				}
