...
namespace FlexUI.Editor
{
	...
	{
		private static readonly string TEMPLATE_NAME = "FlexUI_Templates";
		...
		{
			var text = Resources.Load<TextAsset>(TEMPLATE_NAME).text;

			var result = new Templates();

			var parts = text.Split(new[] { "#TEMPLATE#" }, StringSplitOptions.None);
			foreach (var part in parts)
			{
				var parts2 = part.Split(new[] { '\r', '\n' }, 2, StringSplitOptions.RemoveEmptyEntries);
				if (parts2.Length > 1)
				{
